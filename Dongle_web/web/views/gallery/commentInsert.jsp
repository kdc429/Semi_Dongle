...
<%
	List<GalleryPath> gplist2=(List)request.getAttribute("gplist");
	List<GalleryCommentJoin> gclist2=(List)request.getAttribute("gclist");
...
%>
	...
	<!-- 댓글창 시작 -->
		<ul>
			<%if(gclist2.size()!=0){ %>
				<%for(GalleryCommentJoin g:gclist2){ %>
		...
		<input type="hidden" name="groupNo" id='groupNo' value="<%=gplist2.get(0).getGroupNo() %>"/>
		<input type="hidden" name="galNoC2" id="galNoC2" value="<%=gplist2.get(0).getGalNo() %>"/>
		...
		<input type="hidden" name="albumCode" id='albumCode' value="<%=gplist2.get(0).getAlbumCode()%>"/>
		<input type="hidden" name="galFileNoC2" id='galFileNoC2' value="<%=gplist2.get(0).getGalFileNo()%>"/>
		...
		<%if(relist!=null){ %>
			<form id='reportFrm' name="reportFrm">
		         ...
		         <input type="hidden" id="reportGalNo" name="reportGalNo" value="<%=gplist2.get(0).getGalNo()%>">
		         <input type="hidden" id="reportGroupNo" name="reportGroupNo" value="<%=groupNo%>">
		         <input type="hidden" id="reportMemberNo" name="reportMemberNo" value="<%=gplist2.get(0).getMemberNo()%>">
		      	 <input type="hidden" id="reportAlbumCode" name="reportAlbumCode" value="<%=gplist2.get(0).getAlbumCode()%>">
		...
		/* 댓글 등록 */
			$(function(){
				$('.btn-insert1').click(function(){
					var galNo=$('.comment-editor').find('.modal_comment').siblings('#galNoC2').val();
					var galFileNo=$('.comment-editor').find('.modal_comment').siblings('#galFileNoC2').val();
			...
			$(document).ready(function(){
				$('#btn-report').click(function(e){
					if(<%=loginMember.getMemberNo()%>==<%=gplist2.get(0).getMemberNo()%>){
					 ...
					 var reportNickName='<%=gplist2.get(0).getGroupMemberNickname()%>';
				});
				...
				$('.btn-delete').click(function(){
					...
					if(!confirm("정말로 삭제하시겠습니까?")){return;}
						...
						$.ajax({
							...
							data:{'galCommentNo':$(this).val(),
								'groupNo':<%=groupNo%>,
								'galNo':<%=gplist2.get(0).getGalNo()%>,
								'galFileNo':<%=gplist2.get(0).getGalFileNo()%>,
								'albumCode':'<%=gplist2.get(0).getAlbumCode()%>'
							},
							type:'post',
							dataType:'html',
							success:function(data){
			...
			$(function(){
				...
				$('.btn-reply').on('click',function(e){
					<%if(loginMember!=null){%>
						...
						html+="<input type='hidden' name='albumCodeC3' id='albumCodeC3' value='<%=gplist2.get(0).getAlbumCode()%>'/>";
						...
						div.find('.btn-insert2').click(function(e){
							...
							$.ajax({
								...
								data:{"groupNo":$('#groupNo').val(),
									...
									"galFileNo":'<%=gplist2.get(0).getGalFileNo()%>',
									"galReCommentContent":$('#galReCommentContent').val(),
								},
								type:"post",
								success:function(data){
									if(data!=null)
