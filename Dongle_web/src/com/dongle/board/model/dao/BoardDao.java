...
public class BoardDao {
   ...
   //게시판 댓글쓰기
      ...
      public List<BoardComment> selectBoCommentList(Connection conn, int boardNo, int groupNo)
      {
         ...
         try
         {
            ...
            while(rs.next())
            {
               ...
               bc.setBoCommentReportStatus(rs.getString("bo_comment_report_status"));
      ...
      public int updateBoardCommentReportStatus(Connection conn, int groupNo, int reportBoCommentNo)
      {
          PreparedStatement pstmt = null;
          int result = 0;
          String sql = prop.getProperty("updateBoardCommentReportStatus");
          try {
              pstmt = conn.prepareStatement(sql);
              pstmt.setInt(1, groupNo);
